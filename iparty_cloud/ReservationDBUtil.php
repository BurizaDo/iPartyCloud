...
class ReservationDBUtil{
    ...
    public static function updateStatusAndPayChannel($reservationId, $status, $channel = NULL) {
        $connection = DBUtil::connectDB();
        $ret = mysql_query("update reservation set status='{$status}',pay_channel='{$channel}' where id={$reservationId}", $connection);
        mysql_close();
        return $ret;
    }
